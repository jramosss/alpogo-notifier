...
async def get_all_subscriptions_for_user(
    ...
    return {
        "subscriptions": [
            subscription.place.__data__ for subscription in user.subscriptions
        ]
    }


@router.post("/{place_id}")
async def subscribe_to_place(
    place_id: int, user: Annotated[User, Depends(get_current_user)]
):
...
async def unsubscribe_from_place(
    place_id: int, user: Annotated[User, Depends(get_current_user)]
):
    subscription = Subscription.get_or_none(
        Subscription.user == user, Subscription.place == place_id
    )
