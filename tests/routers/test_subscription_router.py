...
def test_subscribe_to_place(auth_token, place):
    ...
    response = client.post(f"/subscription/{place.id}", headers={"Authorization": f"Bearer {auth_token}"})
    assert response.status_code == 200
    assert response.json() == {"message": f"Subscribed to place {place.name} successfully"}

def test_get_subscriptions_after_subscribing(auth_token, place):
    """Test that a user sees their subscription after subscribing."""
    client.post(f"/subscription/{place.id}", headers={"Authorization": f"Bearer {auth_token}"})
...
def test_subscribe_to_nonexistent_place(auth_token):
    ...
    assert response.status_code == 404  # Assuming Peewee raises `DoesNotExist` for missing places
